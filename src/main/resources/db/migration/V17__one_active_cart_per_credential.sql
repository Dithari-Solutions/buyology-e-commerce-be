...
-- 1. Clean up any existing duplicates: keep the most-recently-updated ACTIVE cart per
--    credential, demote the rest to ABANDONED (their items were already invisible).
UPDATE carts c
...
-- 2. Enforce it going forward. A partial unique index lets CHECKED_OUT/ABANDONED carts
--    accumulate but allows only ONE ACTIVE row per credential — a concurrent second
--    insert now fails the constraint (and is retried/surfaced) instead of silently
--    corrupting the cart.
